...
module readLinkingFile
   ...
   Base.@kwdef mutable struct DATES
      ...
      Date_Start     :: DateTime
      Date_SimStart  :: DateTime
      Date_End       :: DateTime

      Δ∑T_Sim =          0 ::Int64
      Δ∑T_StartSim = 0::Int64
   end

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#		FUNCTION : LINKING_FILES
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   function LINKING_FILE(Path_Hypix::String, NameLinkingFile::String, ProjectHypix::String)

      # PATHS
         Path_LinkingFile = Path_Hypix * "\\data\\INPUT\\Data_Hypix\\" * ProjectHypix * "\\" * ProjectHypix * "_" * NameLinkingFile
      ...
      ## READ DATES ===
         ...
         dateHypix = DATES(Year_Start, Year_Start_Sim, Year_End, Month_Start, Month_Start_Sim, Month_End,Day_Start, Day_Start_Sim, Day_End, Hour_Start, Hour_Start_Sim, Hour_End, Minute_Start, Minute_Start_Sim, Minute_End, Second_Start, Second_Start_Sim, Second_End, Date_Start, Date_SimStart, Date_End, 0, 0)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   function READ_PATH(Data::Matrix{Any}, Header::Matrix{AbstractString}, Path_Input::String, PathName::String; PathAdd=""::String, AllowMissing=false::Bool)
      ...
      try
         ...
         error("HyPix error: $PathName not found in LinkingFile please introduce it")
      end
      ...
      for i = eachindex(Output)
         if Output[i] ≠  ""
            ...
            if PathName ≠ "DISCRETISATION"
               if !(isfile(Output[i]))
                   error("In LinkingFile.csv cannot find " * Output[i])
               end
            else
               mkpath(Path_Input * PathName * "//")
            end
         else
