...
module waterBalance
	...
	function WATERBALANCE(∑T, clim, dateHypix, discret, hydro, N_iRoot::Int64, Nit::Int64, Nz::Int64, Q, ΔSink, ΔT, θ, Ψ)
      ...
      i∑T_CalibrStart = 1::Int64
		...
		# Starting to compute the waterbalance after the warmup period
			while ∑T[i∑T_CalibrStart] < dateHypix.Δ∑T_StartSim
		...
		for iT=i∑T_CalibrStart:Nit
			...
			# Sink term
				∑ΔSink[iT] = ∑ΔSink[iT-1]
				@fastmath @simd for iZ = 1:N_iRoot
